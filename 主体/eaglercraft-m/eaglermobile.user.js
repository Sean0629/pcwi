...
function main() {
    ...
    // Used for crouch mobile control
    window.sprintLock = false;
    // Used for sprint mobile control
    window.keyboardFix = false;
    // keyboardFix ? "Standard Keyboard" : "Compatibility Mode"
    window.inputFix = false;
    // If true, Duplicate Mode
    window.blockNextInput = false;
    // Used for Duplicate Mode 
    window.hiddenInputFocused = false;
    // Used for keyboard display on mobile
    window.canvasTouchMode = 0;
    // Used for canvas touch handling
    /*
        0   Idle
        1   Touch initiated
        2   Primary touch
    ...
    */
        ...
        value: function(type, fn, ...rest) {
            if (type == 'keydown') {
                ...
                _addEventListener.call(this, type, function(...args) {
                    ...
                    // Appends the rest of the function specified by addEventListener
                }, ...rest);
            } else {
    ...
    function mouseEvent(number, state, element, event={
        "clientX": 0,
        "clientY": 0,
        "screenX": 0,
        "screenY": 0
    ...
    document.createElement = function(type, ignore) {
        ...
        if (type == "input" && !ignore) {
            ...
            // Get rid of any left over fileUpload inputs
    ...
    function createTouchButton(buttonClass, buttonDisplay, elementName) {
        ...
        touchButton.addEventListener("touchmove", function(e) {
            e.preventDefault()
        }, false);
    ...
    function insertCanvasElements() {
        ...
        canvas.addEventListener("touchstart", function(e) {
            if (window.canvasTouchMode < 2) {
                ...
                if (window.canvasPrimaryID == null) {
                    ...
                    // TODO: Find out why this isn't redudnant
                }
            }
        }, false);
        ...
        canvas.addEventListener("touchmove", function(e) {
            e.preventDefault()
            // Prevents window zoom when using two fingers
            var primaryTouch;
                ...
                if (window.canvasTouchMode == 1) {
                    ...
                    if (window.canvasTouchMode == 4) {
                        ...
                        canvas.dispatchEvent(new MouseEvent("mousemove",{
                            ...
                            // The top four are used for item position when in GUI's, the bottom two are for moving the camera inGame
            ...
            for (let touchIndex = 0; touchIndex < e.changedTouches.length; touchIndex++) {
                ...
                if (e.changedTouches[touchIndex].identifier == window.canvasPrimaryID) {
                    ...
                    if (window.canvasTouchMode == 2) {
                        ...
                        e.preventDefault();
                        // This prevents some mobile devices from dispatching a mousedown + mouseup event after a touch is ended
                        mouseEvent(2, "mouseup", canvas, primaryTouch)
                    }
        ...
        canvas.addEventListener("touchcancel", canvasTouchEnd, false);
        // TODO: Find out why this is different than touchend
        setButtonVisibility(window.fakelock != null);
        //Updates our mobile controls when the canvas finally loads
        ...
        // We use a div here so can use the targetTouches property of touchmove events. If we didn't it would require me to make an actual touch handler and I don't want to
        ...
        forwardButton.addEventListener("touchmove", function(e) {
            ...
            // We are just hoping that the user will only ever use one finger on the forward button
        ...
        forwardButton.addEventListener("touchend", function(e) {
            keyEvent("w", "keyup");
            // Luckily, it doesn't seem like eagler cares if we dispatch extra keyup events, so we can get away with just dispatching all of them here
            keyEvent("d", "keyup");
            keyEvent("a", "keyup");
            strafeRightButton.classList.remove("active");
            strafeLeftButton.classList.remove("active");
        ...
        inventoryButton.addEventListener("touchend", function(e) {
            keyEvent("shift", "keydown");
            // Sometimes shift gets stuck on, which interferes with item manipulation in GUI's
            keyEvent("shift", "keyup");
            // Sometimes shift gets stuck on, which interferes with item manipulation in GUI's
            keyEvent("e", "keyup");
        }, false);
        ...
        // We hide the input behind a key because display: none and opacity:0 causes issues
        ...
        hiddenInput.addEventListener("beforeinput", function(e) {
            ...
            // Android ignores this and the prevent default, so this will probably be removed in the future
            e.preventDefault(true);
            // We pass a value because we've hijacked the prevent default function to have a "should bypass" boolean value
            let inputData = (e.inputType == "insertLineBreak") ? "return" : (e.data == null ? "delete" : e.data.slice(-1));
            // Saves the last key press. 
            if (!window.lastKey) {
            ...
            if (window.keyboardFix) {
                if (e.inputType == "insertLineBreak") {
                    ...
                    const sliceInputType = e.inputType.slice(0, 1);
                    // Android doesn't constiently dispatch the correct inputType, but most of them either start with i for insert or d for delete, so this dumb solution should be good enough.
                    if (sliceInputType == 'i' && e.data) {
                        ...
                        window.blockNextInput = false;
                        // If we delete a character, there couldn't be a duplicate of the previous key press
                    }
                }
            }
            window.lastKey = inputData
            // Saves the last key pressed
            hiddenInput.value = " "
            //This previously allowed us to have a character to delete, but beforeinput doesn't require this. This does allow us to check wether Duplicate Mode is necessary though
        ...
        placeButton.addEventListener("touchend", function(e) {
        ...
        breakButton.addEventListener("touchstart", function(e) {
            mouseEvent(0, "mousedown", canvas)
        }, false);
        breakButton.addEventListener("touchend", function(e) {
        ...
        throwButton.addEventListener("touchend", function(e) {
            keyEvent("q", "keyup")
        }, false);
